package openapi.spec.generator.openapispecgenerator.utils;
...
public class AvroToJsonUtil {
    ...
    private static final Logger logger = Logger.getLogger(AvroToJsonUtil.class.getName());

    /**
     * @param avscFile input file with avro schema
     * @param jsonFile output file to contain the converted avro schema
     */
    public String convertAvroToJson(String avscFile, String jsonFile) {
        String strReturn ="";
        try {
            ...
            if (JsonSchemaFactory.byDefault().getSyntaxValidator().schemaIsValid(updatedSchema)) {
                strReturn=    JacksonUtils.prettyPrint(jsonSchema.getBaseNode());
            }
